...
export function ContainerConfigEditor({
  ...
  // GUI + GPU 直通注入预览（gui_x11 / gui_wayland / gpu_nvidia / gpu_amd 任一开启时由
  ...
  // 直通预览：value.gui_x11 / value.gui_wayland / value.gpu_nvidia / value.gpu_amd 任一开启时按当前
  // mounts/env 计算引擎将隐式注入的增量（防抖 200ms 合并连击；mounts/env/gui/gpu
  // 变化 → 增量随之变化，需重算）。gui 双开关/gpu_* 是 ContainerConfig 一等字段，随
  // value 传入，故预览入参只传 config。
  useEffect(() => {
    if (!value.gui_x11 && !value.gui_wayland && !value.gpu_nvidia && !value.gpu_amd) {
      setPreview(null);
      return;
    }
    let cancelled = false;
    ...
    // 增量取决于 gui 双开关/gpu_* 开关与用户声明的 mounts/env
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [value.gui_x11, value.gui_wayland, value.gpu_nvidia, value.gpu_amd, value.mounts, value.env]);
    ...
    <div className="config-editor">
      ...
      <section className="config-section">
        ...
        <ContainerPane
          ...
          onGuiX11Change={(gui_x11) => update({ gui_x11 })}
          onGuiWaylandChange={(gui_wayland) => update({ gui_wayland })}
